...
public class Page_Warehouses {
    public static BorderPane createPage(MainWindow mainWindow) {
        ...
        // Pass "Warehouses" to highlight the Warehouses button
        root.setLeft(Panel_Sidebar.createSidebar(mainWindow, "Warehouses"));

        BorderPane rightSide = new BorderPane();
        VBox topPanel = new VBox(
                Panel_Header.createHeader(mainWindow),
                Panel_Title.createTitle("WAREHOUSE")
        );
        rightSide.setTop(topPanel);

        Label contentLabel = new Label("Inventory & Warehouse");
